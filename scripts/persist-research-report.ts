...
function validateSlug(slug: string): void {
  if (!SLUG_RE.test(slug)) {
    throw new Error(
      `Invalid slug "${slug}": must be lowercase kebab-case (a-z, 0-9, hyphens). Path traversal characters are rejected.`,
    );
  }
}
...
export async function runPersistReport(
  input: PersistReportInput,
  ...
  const updatedRow: ResearchReport = { ...row, path: relPath };
