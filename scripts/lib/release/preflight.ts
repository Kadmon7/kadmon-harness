...
import type { ReleaseContext, ReleaseDeps, ReleaseError, PreflightResult } from "./types.js";
...
function checkDirtyTree(cwd: string): ReleaseError | undefined {
  try {
...
function checkVerifyRed(ctx: ReleaseContext, deps: ReleaseDeps): ReleaseError | undefined {
...
function checkEmptyUnreleased(ctx: ReleaseContext): ReleaseError | undefined {
  if (!isUnreleasedEmpty(ctx.cwd)) return undefined;
...
function checkNotOnMain(cwd: string): ReleaseError | undefined {
  try {
...
function checkTagExists(ctx: ReleaseContext, targetVersion: string): ReleaseError | undefined {
  const tagName = `v${targetVersion}`;
  if (!tagExists(ctx.cwd, tagName)) return undefined;
...
export function runPreflight(
  ...
  const results: readonly (ReleaseError | undefined)[] = [
  ...
  const blockers = results.filter((r): r is ReleaseError => r !== undefined);
