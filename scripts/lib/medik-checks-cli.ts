...
export const DEFAULT_REGISTRY: CheckRegistry = new Map<number, CheckModule>([
...
export function parseCliArgs(
  ...
  let cwd = process.cwd();
  let checks: readonly number[] = [...registry.keys()].sort((a, b) => a - b);
    ...
    if (arg === "--cwd" || arg === "--checks") {
      ...
      if (arg === "--cwd") {
        ...
        if (parsed.some((n) => Number.isNaN(n))) {
          ...
          throw new Error(
            `--checks expects a comma-separated list of check numbers, got: ${value}`,
          );
        }
      ...
      throw new Error(
        `unknown argument: ${arg} (usage: --cwd <path> [--checks 10,11,12,13,14])`,
      );
    }
  }
  ...
  if (invalid.length > 0) {
    throw new Error(
      `invalid check id(s): ${invalid.join(", ")} (valid: ${validIds.join(", ")})`,
    );
  }
...
export function resolveProjectHash(cwd: string): string {
  ...
  if (info) return info.projectHash;
...
export async function runChecks(
  options: CliOptions,
  registry: CheckRegistry = DEFAULT_REGISTRY,
): Promise<readonly CliCheckResult[]> {
  ...
  try {
    return options.checks.map((n): CliCheckResult => {
