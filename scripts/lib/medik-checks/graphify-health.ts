...
// Discriminated union: `graphJsonExists: true` structurally REQUIRES a real
// `graphMtimeMs: number` — the invalid state (graph.json present but its
// mtime unknown) is unrepresentable, so no runtime null-guard is needed for
// that branch below.
export type GraphHealthInput =
  | { readonly dirExists: false }
  | { readonly dirExists: true; readonly graphJsonExists: false }
  | {
...
export function evaluateGraphHealth(input: GraphHealthInput): CheckResult {
  if (!input.dirExists) {
    return {
      status: "NOTE",
  ...
  if (input.graphMtimeMs < input.headCommitMs) {
    const staleDays = Math.ceil(
      (input.headCommitMs - input.graphMtimeMs) / MS_PER_DAY,
    );
    return {
      status: "NOTE",
      category: "knowledge-hygiene",
...
function getHeadCommitMs(cwd: string): number | null {
  try {
