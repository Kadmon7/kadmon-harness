...
const JEST_RESULT = (source: string): TestCommandResult => ({
  command: "npx jest",
  runner: "jest",
  source,
});

const VITEST_RESULT = (source: string): TestCommandResult => ({
 ...
 */
export function detectTestCommand(cwd: string): TestCommandResult {
