 ...
 */
function assertContained(targetRoot: string, absolutePath: string): void {
  ...
  if (
    ...
    throw new Error(
      `install-scaffold: refusing to write "${absolutePath}" — resolves ` +
...
function toReportPath(targetRoot: string, absolutePath: string): string {
  return path.relative(targetRoot, absolutePath).split(path.sep).join("/");
}

function scaffoldDir(targetRoot: string, absoluteDir: string): "created" | "skipped" {
...
function scaffoldBacklog(
  targetRoot: string,
  absoluteBacklog: string,
...
export function scaffoldProject(
  ...
  if (!path.isAbsolute(targetRoot)) {
    throw new Error(
      `install-scaffold: targetRoot must be absolute (got "${targetRoot}")`,
    );
  }
  ...
  const templatePath = path.join(
    repoRoot,
    ...BACKLOG_TEMPLATE_RELATIVE_PATH.split("/"),
  );
  if (!fs.existsSync(templatePath)) {
    throw new Error(
      `install-scaffold: BACKLOG.md template not found at ${templatePath} — cannot scaffold BACKLOG.md`,
    );
  }
  ...
  if (templateBytes.length === 0) {
    ...
    throw new Error(
      `install-scaffold: BACKLOG.md template at ${templatePath} is empty ` +
