...
const FRESHNESS_COLUMN: Record<TableName, string> = {
  sessions: "started_at",
...
export function getDbHealthReport(): DbHealthReport {
  const db = getDb();

  const tableCounts = {} as Record<TableName, number>;
  const freshness = {} as Record<TableName, string | null>;
  ...
  const lastSessions: SessionRow[] = db
    .prepare(
      `SELECT id, branch, message_count AS msgs, estimated_cost_usd AS cost,
              started_at, ended_at
       FROM sessions
  ...
  const hookEvents24h: HookStat[] = db
    .prepare(
      `SELECT hook_name, COUNT(*) AS n,
  ...
  const agentInvocations24h: AgentStat[] = db
    .prepare(
  ...
  const costEvents24h: CostStat[] = db
    .prepare(
      `SELECT model, COUNT(*) AS n, SUM(estimated_cost_usd) AS total_usd
       FROM cost_events
       WHERE timestamp > datetime('now', '-1 day')
       GROUP BY model
...
function detectAnomalies(
  ...
  for (const t of TABLES) {
    if (t === "sync_queue") continue;
    if (tableCounts[t] === 0) anomalies.push(`table_empty: ${t}`);
  }
  ...
  if (tableCounts.sessions > 0 && latest) {
    ...
    if (!Number.isNaN(parsed)) {
      ...
      if (days > STALE_SESSIONS_DAYS) {
        anomalies.push(
          `sessions_stale: latest ${days}d old (threshold ${STALE_SESSIONS_DAYS}d)`,
        );
      }
    }
  }
