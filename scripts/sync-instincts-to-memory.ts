...
import type { Instinct } from "./lib/types.js";
...
function instinctToTopicFile(instinct: Instinct): string {
...
function buildMemoryIndex(instincts: Instinct[]): string {
  const lines: string[] = [];
  ...
  if (instincts.length === 0) {
    lines.push(
      "No active instincts yet. Run `/learn` after a productive session.",
    );
    return lines.join("\n");
  }

  lines.push(
    `${instincts.length} active instinct(s) synced from Kadmon Harness.`,
  );
  lines.push("");
  ...
  for (const inst of instincts) {
    const slug = inst.pattern
      ...
      .slice(0, 50);
    const bar =
      inst.confidence >= 0.7 ? "HIGH" : inst.confidence >= 0.5 ? "MED" : "LOW";
    lines.push(
      `- [${bar}] [${inst.pattern}](instinct-${slug}.md) — ${inst.action.slice(0, 80)}`,
    );
  }
...
export async function syncInstinctsToMemory(
  cwd?: string,
): Promise<{ synced: number; memoryDir: string }> {
