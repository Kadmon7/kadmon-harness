...
export interface DogfoodReport {
  ...
  summary: { passed: number; failed: number; total: 22 };
}
 ...
 */
export function checkSandbox(sandboxPath: string): SandboxStatus {
 ...
 */
export function buildEventSequence(sessionId: string, sandboxCwd: string): SimulatedEvent[] {
  const events: SimulatedEvent[] = [];

  // Helper: get hooks for an event+matcher key
  function hooksFor(event: string, matcher: string): string[] {
...
function runHookDirect(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    KADMON_RUNTIME_ROOT: undefined as unknown as string,
    // Use real DB so we can verify persistence afterward
    KADMON_TEST_DB: undefined as unknown as string,
 ...
 */
async function queryPersistedHookEvents(sessionId: string): Promise<Set<string>> {
 ...
 */
export async function runPluginModeDogfood(sandboxPath: string): Promise<DogfoodReport> {
  ...
  if (!status.hasGitRemote && fs.existsSync(sandboxPath)) {
    try {
      ...
      try {
        execFileSync(
          "git",
          ["remote", "add", "origin", "git@github.com:fake/kadmon-plugin-sandbox.git"],
          { cwd: sandboxPath, stdio: "pipe", timeout: 3000 },
        );
      } catch {
  ...
  const hookResults = new Map<string, number[]>();
  ...
  // Step 5: Build report
  const hooksInvoked: HookInvocationResult[] = [];
  const hooksNotDisparados: string[] = [];
...
export function formatReport(report: DogfoodReport): string {
  ...
  lines.push("");
  lines.push(
    `Cleanup: DELETE FROM sessions WHERE id = '${report.sessionId}';`,
  );
...
if (isCli) {
  ...
  const sandboxPath =
    sandboxArgIdx !== -1 && process.argv[sandboxArgIdx + 1]
