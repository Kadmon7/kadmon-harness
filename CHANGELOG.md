...
- **Project-agnostic stack: 9/11 commands** — `/skanner` (plan-031, harness\|web\|cli profile), `/doks` (plan-032), `/medik` (plan-033), `/chekpoint` (plan-034) join previously project-agnostic `/kompact`, `/abra-kdabra`, `/skavenger`, `/almanak`, `/nexus`. `/forge` + `/evolve` remain harness-only by design (instinct/observability infrastructure + harness self-optimization).
...
- **Metrics (refreshed 2026-04-26 post-merge)**: **1115 tests passing** (89 test files) · 22 hooks + 9 shared modules · 16 agents · 48 skills · 11 commands · 19 rules · 7 DB tables · 14 `/medik` checks · 9/11 commands cross-project (forge + evolve harness-only by design).
