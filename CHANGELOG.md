...
### Added
- **Kadmon OS web dashboard** ([plan-039](docs/plans/plan-039-web-dashboard.md)) — a local, read-only web view of the harness: `npm run dashboard:web` starts a `node:http` server bound to `127.0.0.1` (port via `KADMON_DASHBOARD_PORT`, Zod-validated 1024-65535) serving a single dark HTML page over 3 routes. Surfaces a live catalog scan (agents / skills / commands / hooks / tests read from disk, never from doc counts) alongside SQLite telemetry: instincts with confidence and a promote-ready highlight at `>= 0.7`, sessions plus orphan count, cost events, hook health against latency budgets, and agent-usage stats. New `scripts/dashboard-web.ts` + `scripts/lib/dashboard-web-data.ts` (`buildCatalog` / `buildTelemetry` / `findActiveSessionDir`, the last one aware of `observations.archive.jsonl` mtime) + `scripts/dashboard-web/index.html` with a 10s poll. Six TDD steps executed by feniks in an isolated worktree (`300d0c4`), then styled to **Kadmon Brand Foundation v0.2** (`df32771`) — Deep Space ground, Kadmon Gold as structure-only accent, a semantic state scale kept separate from gold (over-budget rows `state-crit`, orphan count red only when `> 0`), tabular numerals, mono-dash empty states, focus-visible outline, reduced-motion-safe pulse. Full-tier `/chekpoint` at merge closed 3 ts-reviewer WARNs and 1 spektr MEDIUM (`d395936`): a pre-`listen()` `server.on('error')` handler that reports `EADDRINUSE` in one line instead of an unhandled stack, a constant generic `{ error: "internal error" }` 500 body with detail kept server-side only, and a new `knownOutcomes` field on `getAgentInvocationStats` so `successRate` resolves to `null` when no outcomes are known instead of colliding with a real 0. **The server is a foreground local process — it does not survive a reboot and is not registered as a service.**
- **`/release` upgrade-advisory phase** ([ADR-037](docs/decisions/ADR-037-release-command.md) D7 Amendment) — after the tag, `/release` classifies `git diff --name-only v<prev>..HEAD` into ADR-010 distribution territories and prints the exact commands consumers run to pull the release: **plugin update** (`/plugin marketplace update` → `/plugin update` → `/reload-plugins`) when `.claude/{agents,skills,commands,hooks}/` or `.claude-plugin/` changed; **install re-run** (`install.ps1 -ForcePermissionsSync` / `./install.sh`) when `.claude/rules/`, `.claude/settings.json`, or `install.{sh,ps1}` changed; **re-drop onboarding catalog** when `docs/onboarding/{reference_kadmon_harness.md,CLAUDE.template.md}` changed. Closes the README gap where "Updating the harness later" documented only the clone+install refresh, never the `/plugin update` step (README now carries a Part-1 plugin-update block). New module `scripts/lib/release/upgrade-advisory.ts` (release subsystem 7 → 8 concern modules) — pure `git diff` read, zero mutation, best-effort (a git failure yields a neutral advisory, never blocks a release). TDD via feniks, 39 new tests.
- **`hebrew-native-copy` skill** (`4dae117`, skills 52 → 53) — rewrites Hebrew marketing and UI copy from machine-translated register to native, gender-safe, professional-warm Israeli Hebrew, targeting the two tells that mark Hebrew as translated: gendered second-person address (the default-masculine trap that excludes female readers) and English calques. Ships `SKILL.md` (workflow + honesty contract) and `references/hebrew.md` (two gender strategies — singular-homograph-neutral and plural — plus de-calque rules, register control, RTL/typography, worked before/after), generalizing to other languages via `references/<language>.md`. Created and validated in the Kadmon7Cowork fork against real landing strings, ported up here as the source of truth. Depends on `copy-deslop`.
- **Semaphore + claim conventions in the `/sprint` coordination template** (`d4ae94a`) — `WORK_COORDINATION.template.md` now carries the team's two-axis model: a readiness semaphore (blocked / waiting-external / ready / deferred) kept orthogonal to ownership (Owner + Channel), plus an oldest-commit-wins claim tie-break. Every project instantiating `/sprint` coordination inherits it.
- **v2.0 ECC-delta port roadmap** (`1aac06f`, [docs/roadmap/v2.0-ecc-delta-ports.md](docs/roadmap/v2.0-ecc-delta-ports.md)) — analysis of `affaan-m/ECC` v1.8.0..v2.0.0 since the 2026-03-05 port point via 4 parallel analyst agents: 216 new skills → 12 port-worthy, 51 agents + 60 commands → ~12 signal files, 92 rules → substance in fastapi/react/RN/web. Learning-system parity holds. Waves 0-4; Wave 0 shipped inside this release as the observations-archive fix.
- **`kontinuum` freeze anchored to a commit sha + thaw re-sync** (`a38671c`) — the session-handoff skill now records the exact commit sha at FREEZE time, and THAW runs `git pull --ff-only` first, prints a drift report of what landed while the session was frozen, and reconciles `WORK_COORDINATION.md` claims when the repo has one. *(Note: landed from a parallel session during the cut, after the `[Unreleased]` consolidation — narrated here post-tag because the commit is inside `v1.5.0`'s history and the CHANGELOG must describe what the tag actually contains.)*

### Fixed
- **`/forge` and `/kompact` went blind on long sessions** (`09ea842`, Wave 0 of the ECC-delta roadmap) — Phase 5 of `session-end-all` unlinked `observations.jsonl` on **every** `Stop` once `messageCount >= 20`, and `Stop` fires per turn rather than per session, so past message 20 each turn wiped the file. `/forge` reads that ephemeral JSONL rather than the DB and therefore only ever saw the last turn, which is why it returned empty after dense sessions. The second victim was worse: `/kompact`'s audit reads the same file, under-reporting tool calls by ~20x, so its "productive session (>30 tool calls) without /forge" safety check never fired on exactly the long sessions it exists to protect — the bug disabled its own guard. Fixed with the ECC `f720885c` archive pattern: Phase 5 now appends the live file to `observations.archive.jsonl` before unlinking and **retains the live file on append failure** (retry next Stop, logged via `logHookError`); Phase 1 builds a combined archive+live view for extraction, summaries, session persistence and cross-turn agent pairing; `forge-pipeline` and the `/kompact` audit one-liners read archive+live. Phase 3 Fallback-2 cost estimation stays live-only to avoid inflated cost events. No data was ever lost — the reader was pointed at the wrong source. TDD via feniks (6 red tests first).
- **Five silent-swallow catch sites in `scripts/lib/release/`** (`dd2ad52`) — `tag.ts` `tagExists`, `orchestrate.ts` `isVersionAlreadyBumped`, `backlog-prune.ts` `readChangelogText`, `status-flips.ts` `readFileSafe`, and `upgrade-advisory.ts` `defaultRunDiff` each discarded their error and returned a fallback with zero logging, violating `common/patterns.md`. The swallow itself is deliberate at every site (read-only best-effort paths), so every fallback and all control flow are byte-identical — the only change per site is binding `e: unknown` and logging through the existing `log()` before the same return. Note for future maintainers: the diff logs `e.message`, **not** `e.stdout`, which for `git tag -l` holds the tag list — do not "improve" this to log the whole error object.
- **Contract test red on `main`** (`04ff1df`) — `4dae117` synced the 52 → 53 skills count into `CLAUDE.md` and `README.md` but missed the hardcoded `expectedCounts` assertion in `tests/plugin/manifest-schema.test.ts`, leaving `main` red from 2026-07-15 to 07-16 while `WORK.md` claimed the suite was green. The hardcoded literal stays deliberately: deriving it from the same filesystem the test reads would make the assertion tautological and destroy the only mechanism that forces a human to notice a component changed.

### Changed
- **Vitest no longer sweeps agent worktrees** (`59bdf9a`) — `.claude/worktrees/<agent>/` are full checkouts inside the repo, so a main-tree run collected each worktree's `tests/` **and** its compiled `dist/tests/`, triplicating the suite (331 files / 79 spurious failures observed live with one active worktree) and cross-contaminating tmpdir fixtures; the pre-existing `dist/**` exclude only matched the root `dist`. Now excluded in `vitest.config.ts` and gitignored so `git add -A` cannot swallow them.
- **Documentation Layers 1-3 synced with v1.4.0** (`816d0b1`) — the v1.4.0 cut's `/doks` step was scoped to count-sync only; this closes the rest plus five previously untracked drifts. The README skills collapsible said 48 and enumerated 32 of the real 53 (both fixed, descriptions read from each skill's own frontmatter); the README `latest:` narrative still described v1.3.0; `/evolve` step 6 was still tagged EXPERIMENTAL in 3 places 3 months after being promoted to accepted; `/doks` was described as "4-layer" in 5 files despite the 2026-04-26 Amendment to ADR-032 taking rules out of scope; `agent-authoring` said `/medik` runs 8 checks when it runs 16; `/release` was absent from every orchestration list; and the consumer-facing onboarding catalog sat at 52 skills. Verified by **set-equality** against `ls .claude/skills/*/`, not by counting — a count that matches with a set that does not is still drift. Two rules NOTEs were deliberately left unapplied (rules are hand-curated via ADR).
- **Working docs** — `BACKLOG.md` absorbed the review findings, orphan pendientes and root-caused bugs this cycle surfaced (`53490c1`, `d657b13`, `67f0c9c`, `0099483`, `fdb2dc4`, `4da37a2`); `CORRECTIONS.md` gained **C-006**, widening C-004 from "memories decay" to the full class — audits, working docs, ADR checkboxes, generated graphs and sub-agent reports are all point-in-time claims, so re-derive before citing.

## [1.4.0] — 2026-07-15

### Added
- **`/release` command** (commands 11 → 12) — [ADR-037](docs/decisions/ADR-037-release-command.md), [plan-037](docs/plans/plan-037-release-command.md). One human-invoked command collapses the manual release-hygiene steps: version bump (`plugin.json` canonical + `package.json` mirror), CHANGELOG `[Unreleased]` → dated-heading consolidation, BACKLOG `[x]` prune (prune-only + unnarrated-warn), ADR/plan/roadmap status-flip **proposals** (output-only — no auto-flip, no WORK.md append, D5), and an idempotent annotated tag. Logic lives in a new `scripts/lib/release/` subsystem (`types.ts` + 7 concern modules — version-bump, changelog, backlog-prune, status-flips, tag, preflight, orchestrate). Composes `/doks` for Layer-1 count sync (never recomputes counts itself, D6) then re-verifies so the `expectedCounts` contract test gates residual drift. Five D4 refusal gates (dirty-tree / verify-red / empty-Unreleased / not-on-main / existing-tag) with collect-all reporting; idempotent re-run converges (committed-but-untagged → tag-only). No-push default — `--push` is an explicit human publish step (D3). Direct no-agent command. The autonomous-routine framing in the ADR body is retired (2026-07-13 Amendment) — `/release` has no scheduled or unattended path.
