 ...
 */
export function parseSettings(settingsPath) {
 ...
 */
export function generateStdin(hook) {
 ...
 */
export function runHook(hook, stdin) {
 ...
 */
export function reportTable(results) {
    const lines = [];
...
if (isCli) {
    ...
    for (const hook of hooks) {
        ...
        process.stdout.write(`${icon} ${hook.name.padEnd(28)} ${hook.event.padEnd(22)} ${(hook.matcher || "(empty)").padEnd(14)} exit=${exitStr}  ${result.durationMs}ms  ${status} (${reason})\n`);
    }
    console.log("\n" + "-".repeat(80));
    const passCount = results.filter((r) => classify(r).status === "pass").length;
    const failCount = results.length - passCount;
    console.log(`Summary: ${passCount} pass, ${failCount} fail, 0 error out of ${results.length}`);
