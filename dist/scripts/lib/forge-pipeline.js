...
import { getActiveInstincts, upsertInstinct, getCrossProjectPromotionCandidates, } from "./state-store.js";
import { promoteToGlobal } from "./instinct-manager.js";
import { evaluatePatterns, loadPatternDefinitions, } from "./pattern-engine.js";
...
function assertSafeSessionId(sessionId) {
    if (!SAFE_ID_PATTERN.test(sessionId)) {
        throw new Error(`forge-pipeline: unsafe sessionId "${sessionId}" — must match /^[A-Za-z0-9_-]+$/`);
    }
}
...
// ─── Main pipeline ───
export async function runForgePipeline(opts) {
    ...
    const { create, reinforce } = projectInMemory(candidates, existing, projectHash, sessionId);
...
export function applyForgePreview(preview, _opts) {
    ...
    const scopeIds = Array.from(new Set(preview.would.scopePromote.map((s) => s.instinctId)));
    if (scopeIds.length > 0)
        void promoteToGlobal(scopeIds);
}
function readObservationsForSession(sessionId) {
    const obsPath = path.join(os.tmpdir(), "kadmon", sessionId, "observations.jsonl");
...
function extractCandidates(toolSeq, lines) {
    if (toolSeq.length === 0 && lines.length === 0)
        return [];
    const defsPath = fileURLToPath(new URL("../../.claude/hooks/pattern-definitions.json", import.meta.url));
...
function projectInMemory(candidates, existing, projectHash, sessionId) {
    const byPattern = new Map(existing.map((i) => [i.pattern, i]));
    const create = [];
    const reinforce = [];
    const now = nowISO();
    for (const c of candidates) {
        ...
        if (match) {
            ...
            const after = {
                ...match,
                confidence: Math.min(MAX_CONFIDENCE, Math.round((match.confidence + REINFORCE_DELTA) * 100) / 100),
...
function applyProjectionInMemory(existing, create, reinforce) {
    const replaced = new Map(reinforce.map((r) => [r.before.id, r.after]));
    const projected = existing.map((i) => replaced.get(i.id) ?? i);
    return [...projected, ...create];
}
function evaluateRecommendations(instincts) {
    const promote = [];
    const prune = [];
    for (const inst of instincts) {
        if (inst.status !== "active")
            continue;
        if (inst.confidence >= PROMOTE_CONFIDENCE &&
            inst.occurrences >= PROMOTE_OCCURRENCES) {
            promote.push(inst);
            continue;
        }
        const lowConfidence = inst.confidence < PRUNE_LOW_CONFIDENCE &&
...
export function computeClusterReport(instincts, projectHash, sessionId) {
    ...
    const promotableCount = active.filter((i) => i.confidence >= PROMOTE_CONFIDENCE && i.occurrences >= PROMOTE_OCCURRENCES).length;
    return {
...
function buildCluster(domain, members) {
    ...
    const id = crypto
        ...
        .digest("hex")
        .slice(0, 8);
    const meanConfidence = members.reduce((s, m) => s + m.confidence, 0) / members.length;
    const totalOccurrences = members.reduce((s, m) => s + m.occurrences, 0);
    const contradictionCount = members.reduce((s, m) => s + m.contradictions, 0);
    const distinctSessions = new Set(members.flatMap((m) => m.sourceSessions)).size;
    let suggestedCategory = "OPTIMIZE";
