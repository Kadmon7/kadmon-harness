...
export function createInstinct(projectHash, pattern, action, sourceSessionId, scope = "project", domain) {
    const now = nowISO();
    const instinct = {
        id: generateId(),
        projectHash,
...
export function reinforceInstinct(id, sessionId) {
    ...
    const updated = {
        ...existing,
        confidence: Math.min(0.9, Math.round((existing.confidence + 0.1) * 100) / 100),
...
export function contradictInstinct(id) {
    const existing = getInstinct(id);
    if (!existing)
        return null;
    const contradictions = existing.contradictions + 1;
    const status = contradictions > existing.occurrences
        ? "contradicted"
        : existing.status;
    const updated = {
        ...existing,
...
export function promoteInstinct(id, skillName) {
    const existing = getInstinct(id);
    if (!existing)
        return null;
    if (existing.confidence < 0.7 || existing.occurrences < 3)
        return null;
    const updated = {
        ...existing,
        status: "promoted",
...
export function pruneInstincts(projectHash) {
    const db = getDb();
    const now = new Date();
    const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString();
    // Archive contradicted instincts older than 7 days
    db.prepare("UPDATE instincts SET status = 'archived', updated_at = @now WHERE project_hash = @ph AND status = 'contradicted' AND updated_at < @cutoff").run({ ph: projectHash, now: nowISO(), cutoff: sevenDaysAgo });
    // Archive low-confidence, low-occurrence instincts
    db.prepare("UPDATE instincts SET status = 'archived', updated_at = @now WHERE project_hash = @ph AND status = 'active' AND confidence < 0.2 AND occurrences < 2").run({ ph: projectHash, now: nowISO() });
    // Count how many were archived (approximate — we count all archived for this project)
    const row = db
        .prepare("SELECT COUNT(*) as cnt FROM instincts WHERE project_hash = ? AND status = 'archived'")
        .get(projectHash);
 ...
 */
export function decayInstincts(projectHash, now = new Date()) {
    const db = getDb();
    const rows = db
        .prepare("SELECT id, confidence, updated_at, last_observed_at FROM instincts WHERE project_hash = ? AND status = 'active'")
        .all(projectHash);
    const nowIso = now.toISOString();
    const updateStmt = db.prepare("UPDATE instincts SET confidence = @confidence, updated_at = @now WHERE id = @id");
    let decayed = 0;
    let totalLoss = 0;
    db.transaction(() => {
        for (const row of rows) {
            ...
            const newConfidence = Math.round(Math.max(0, row.confidence - decayAmount) * 100) / 100;
 ...
 */
export function promoteToGlobal(instinctIds) {
    let promoted = 0;
    getDb().transaction(() => {
...
export function getInstinctSummary(projectHash) {
    const instincts = getActiveInstincts(projectHash);
    if (instincts.length === 0)
        return "No active instincts.";
    const lines = [`### Active Instincts (${instincts.length})`];
    for (const inst of instincts.slice(0, 5)) {
        lines.push(`- [${inst.confidence.toFixed(1)}×${inst.occurrences}]${inst.domain ? ` (${inst.domain})` : ""} ${inst.pattern} → ${inst.action}`);
    }
