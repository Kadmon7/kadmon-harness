 ...
 */
function buildTargetPath(type, slug) {
    const joined = type === "skill"
 ...
 */
function deriveComplexity(memberCount) {
    if (memberCount <= 2)
        return "S";
    if (memberCount <= 5)
        return "M";
 ...
 */
function deriveConfidence(meanConfidence) {
...
// ─── Spec builder ───
function buildSpec(type, cluster) {
 ...
 */
export async function runEvolveGenerate(opts) {
...
function computeSourceWindow(reports, now) {
    if (reports.length === 0) {
...
function loadTemplate(templateName) {
    const templateDir = fileURLToPath(new URL("./evolve-generate-templates", import.meta.url));
...
function renderTemplate(template, tokens) {
    return Object.entries(tokens).reduce((content, [key, value]) => content.replace(new RegExp(`\\{\\{${key}\\}\\}`, "g"), value), template);
}
/**
 ...
 */
export function renderProposalToMarkdown(proposal, now = new Date()) {
 ...
 */
export function applyEvolveGenerate(preview, approvals, opts) {
    const approved = preview.proposals.filter((p) => approvals.approvedIndices.includes(p.index));
    const written = [];
    ...
    for (const proposal of approved) {
        ...
        if (!absolutePath.startsWith(dotClaudeDir + path.sep) && absolutePath !== dotClaudeDir) {
            errors.push(`Path traversal rejected: "${proposal.targetPath}" resolves outside .claude/`);
            continue;
        }
    ...
    for (const { proposal, absolutePath } of tasks) {
        try {
            ...
            errors.push(`Failed to write ${absolutePath}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return { written, pluginInvocations, collisions, errors };
}
