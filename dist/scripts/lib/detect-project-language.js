...
const TS_LINT_EXTENSIONS = new Set([".ts", ".tsx", ".js", ".jsx"]);
...
const CONTENT_SCAN_EXCLUDED_EXTENSIONS = new Set([
    ".md",
    ".txt",
    ".rst",
    ".json",
...
function isDiffProductionSource(normalized) {
    const ext = path.extname(normalized).toLowerCase();
    const hasCodeExt = TS_EXTENSIONS.has(ext) || ext === PY_EXTENSION || ext === ".js" || ext === ".jsx";
    if (!hasCodeExt)
        return false;
    if (isDiffTestFile(normalized))
        return false;
...
function isDiffUnknownExtension(normalized) {
    const ext = path.extname(normalized).toLowerCase();
    if (!ext)
        return true;
...
function scanDiffContentKeywords(normalized, content, keywords) {
    const ext = path.extname(normalized).toLowerCase();
    if (CONTENT_SCAN_EXCLUDED_EXTENSIONS.has(ext))
        return [];
 ...
 */
export function getDiffScope(stagedFiles, fileContents) {
    ...
    for (const norm of normalized) {
        ...
        if (!isDiffDocFile(norm))
            allDocs = false;
        if (TS_LINT_EXTENSIONS.has(ext))
            hasTsLint = true;
        if (TS_EXTENSIONS.has(ext))
            hasTsOnly = true;
        if (ext === PY_EXTENSION)
            hasPy = true;
        if (isDiffConfigFile(norm))
            hasConfig = true;
        if (isDiffTestFile(norm))
            hasTest = true;
        if (isDiffProductionSource(norm))
            hasProduction = true;
        if (isDiffUnknownExtension(norm))
            hasUnknown = true;
