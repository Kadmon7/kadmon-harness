 ...
 */
export function defaultGhRunner(args, opts) {
    try {
...
// ---- Public probes ----
export function checkGhAvailable(runner = defaultGhRunner) {
    const r = runner(["--version"]);
    return r.exitCode === 0;
}
export function checkGhAuthenticated(runner = defaultGhRunner) {
    const r = runner(["auth", "status"]);
    return r.exitCode === 0;
}
// ---- Rate-limit parsing ----
function parseRateLimitJson(body) {
    ...
    const reset = resetEpoch > 0 ? new Date(resetEpoch * 1000).toISOString() : "";
...
// ---- Per-kind fetchers ----
function fetchIssuesOrPrs(runner, repo, kind, limit, timeoutMs) {
    ...
    return [
        {
            ...
            url: parsed.html_url ?? `https://github.com/${repo}/blob/HEAD/README.md`,
            body,
        },
    ];
}
function fetchChangelog(runner, repo, timeoutMs) {
...
function fetchDiscussions(runner, repo, limit, timeoutMs) {
    ...
    const query = "query($owner: String!, $name: String!, $limit: Int!) { repository(owner: $owner, name: $name) { discussions(first: $limit) { nodes { title url body } } } }";
    const r = runner([
        "api",
        "graphql",
        "-f",
...
export async function fetchGitHubRepoContent(opts) {
    ...
    if (rlRes.exitCode !== 0) {
        ...
        const hint = /ENOENT|not found|not installed/i.test(msg)
...
if (invokedDirectly) {
    ...
    if (!repo) {
        process.stderr.write("usage: github-research <owner/name> [kinds-comma-separated]\n");
        process.exit(1);
    }
    const ALL_KINDS = [
        "issues",
    ...
    const isKnownKind = (k) => ALL_KINDS.includes(k);
    ...
    if (unknown.length > 0) {
        process.stderr.write(`unknown kinds: ${unknown.join(", ")} (valid: ${ALL_KINDS.join(", ")})\n`);
        process.exit(1);
    }
