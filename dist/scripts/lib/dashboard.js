...
import { getActiveInstincts, getPromotableInstincts, getRecentSessions, getCostSummaryByModel, getInstinctCounts, getHookEventStats, getAgentInvocationStats, getDb, } from "./state-store.js";
// ─── ANSI helpers ───
const BOLD = "\x1b[1m";
const DIM = "\x1b[2m";
const RESET = "\x1b[0m";
...
function miniBar(value, max, width = 8, log = false) {
    if (max <= 0 || value <= 0)
        return "\u2591".repeat(width);
    let ratio;
    if (log) {
...
function getHookStatRows(projectHash) {
    return getHookEventStats(projectHash);
}
function getAgentStatRows(projectHash) {
...
// ─── Health Score ───
function computeHealthScore(projectHash, sessions, hookStats, agentStats) {
    let score = 100;
...
export function renderDashboard(projectHash, events) {
    ...
    lines.push(`${BOLD}${CYAN}  \u2551  \u{1F9E0}  KADMON HARNESS DASHBOARD        \u2551${RESET}`);
    lines.push(`${BOLD}${CYAN}  \u255A${"═".repeat(44)}\u255D${RESET}`);
    // Fetch all data
    ...
    // Health Score
    const health = computeHealthScore(projectHash, sessions, hookStats, agentStats);
    const grandTotal = modelCosts.reduce((sum, r) => sum + r.totalCost, 0);
    const totalSessions = sessions.length;
    lines.push("");
    lines.push(`  ${health.color}${BOLD}\u{1F3AF} Health: ${health.score}/100 ${health.label}${RESET}` +
        `    ${DIM}|${RESET}  \u{1F4CA} ${totalSessions} sessions` +
        `    ${DIM}|${RESET}  \u{1F4B0} $${grandTotal.toFixed(2)} total`);
    lines.push("");
    lines.push(separator());
    // ═══ 1. Instincts ═══
    const countLabel = counts.promotable > 0
    ...
    if (instincts.length === 0) {
        ...
        for (const row of instincts) {
            ...
            const promoTag = row.isPromotable
                ...
                : "";
            lines.push(`  ${row.bar}  ${row.pattern} ${DIM}(${row.occurrences}x)${RESET}${promoTag}`);
        }
    }
    lines.push("");
    lines.push(separator());
    ...
    if (sessions.length === 0) {
        ...
        lines.push(`  ${DIM}Date        Branch              Files  Msgs  Cmps  Duration  Cost${RESET}`);
        ...
        for (const row of sessions) {
            ...
            lines.push(`  ${row.date}  ${branch} ${files} ${msgs} ${cmps}  ${dur}  ${row.cost} ${DIM}${costBar}${RESET}${liveTag}`);
        }
    ...
    if (modelCosts.length === 0) {
        ...
        lines.push(`  ${DIM}Model              Sessions  Tokens In  Tokens Out   Cost${RESET}`);
        ...
        lines.push(`  ${"Total".padEnd(18)} ${" ".repeat(8)} ${" ".repeat(10)} ${" ".repeat(10)}  ${GREEN}$${grandTotal.toFixed(2)}${RESET}`);
    }
    lines.push("");
    lines.push(separator());
    ...
    if (hookStats.length === 0) {
        ...
        lines.push(`  ${DIM}Hook                     Total  Blocks  Avg ms  Status${RESET}`);
    ...
    if (agentStats.length === 0) {
        ...
        lines.push(`  ${DIM}Agent                Total  Avg Duration  Fail%  Status${RESET}`);
        for (const row of agentStats) {
            ...
            if (row.failureRate === 0)
                status = "OK";
            else if (row.failureRate < 0.3)
                status = "WARN";
            else
                status = "FAIL";
            lines.push(`  ${name}  ${total} ${avgDur} ${failPct}  ${statusBadge(status)}`);
        }
    }
    lines.push("");
    lines.push(separator());
    ...
    // Footer
    lines.push("");
    lines.push(`  ${DIM}v1.0 | ${new Date().toISOString().slice(0, 19)} | project: ${projectHash.slice(0, 8)}${RESET}`);
    lines.push("");
    return lines.join("\n");
}
