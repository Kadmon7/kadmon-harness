 ...
 */
function escapePwshDoubleQuoted(s) {
    return s
        .replace(/`/g, "``")
        .replace(/\$/g, "`$")
        .replace(/"/g, '`"');
}
function renderPluginCacheRemediation(rootDir) {
...
function renderRepoCloneRemediation() {
    return [
...
export function renderRemediationBanner(issues, context) {
    const actionable = issues.filter((s) => ACTIONABLE_STATES.includes(s.state));
