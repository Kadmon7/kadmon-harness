...
export const MEDIA_URL_RE = /^https?:\/\/(?:www\.|m\.)?(?:(?:youtube\.com\/watch\?v=|youtu\.be\/)([\w-]{11})|vimeo\.com\/\d+|soundcloud\.com\/[^/\s]+\/[^/\s]+|clips\.twitch\.tv\/[^/\s]+|twitch\.tv\/videos\/\d+|(?:twitter|x)\.com\/[^/\s]+\/status\/\d+|tiktok\.com\/@[^/\s]+\/video\/\d+|archive\.org\/details\/[^/\s]+|dailymotion\.com\/video\/[^/\s]+)/i;
...
export async function fetchYouTubeTranscript(opts) {
    ...
    const canonicalUrl = youtubeVideoId !== null ? buildCanonicalUrl(youtubeVideoId) : opts.url;
    ...
    if (!checkYtDlp()) {
        return {
            ok: false,
            source: "error",
            error: "yt-dlp not found. Install: winget install yt-dlp (Windows) / brew install yt-dlp (macOS) / pip install yt-dlp",
        };
    }
    ...
    try {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            process.stderr.write(JSON.stringify({ warning: "tempdir cleanup failed", dir: tempdir, error: message }) + "\n");
        }
    }
}
