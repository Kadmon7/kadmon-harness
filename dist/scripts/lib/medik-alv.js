 ...
 */
function buildEscapeVariants(p) {
 ...
 */
export function redactSensitivePaths(text, cwd, homedir, additionalPaths = []) {
    let result = text;
    ...
    result = result.replace(/[A-Za-z]:(?:\\{1,4}|\/)[\w.\- ]+(?:(?:\\{1,4}|\/)[\w.\- ]+){2,}/g, "<path>");
    result = result.replace(/(?<![\w/])\/(?:[\w.\- ]+\/){2,}[\w.\- ]+/g, "<path>");
    return result;
}
 ...
 */
function collectAdditionalRoots(diagEntries, hookErrors) {
    const roots = new Set();
    for (const e of diagEntries) {
        const parsed = rootDirSchema.safeParse(e);
        if (parsed.success)
            roots.add(parsed.data.rootDir);
    }
 ...
 */
export function generateAlvReport(cwd) {
 ...
 */
export function writeAlvReport(cwd, outputDir) {
    ...
    if (!insideCwd && !insideTmp) {
        throw new Error(`writeAlvReport: outputDir must be inside cwd or os.tmpdir() (got: ${outputDir ?? "(default cwd)"})`);
    }
    const safeName = new Date().toISOString().slice(0, 16).replace(/:/g, "");
    let fullPath = path.join(resolvedOut, `diagnostic-${safeName}.txt`);
    let fd;
    try {
        fd = fs.openSync(fullPath, fs.constants.O_WRONLY | fs.constants.O_CREAT | fs.constants.O_EXCL, 0o600);
    }
    catch (err) {
        if (err.code === "EEXIST") {
