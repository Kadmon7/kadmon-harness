...
function findNextHeadingIndex(content, fromIndex) {
...
function releasedHeadingRegex(target) {
    return new RegExp(`^## \\[${escapeRegExp(target)}\\]`, "m");
}
function computeConsolidation(content, target, date) {
    ...
    const newContent = content.slice(0, unreleasedIdx) +
 ...
 */
export function isUnreleasedEmpty(cwd) {
    const content = readChangelog(cwd);
    const unreleasedIdx = content.indexOf(UNRELEASED_HEADING);
    if (unreleasedIdx === -1)
        return true; // no [Unreleased] section — nothing to release
    const afterHeadingIdx = unreleasedIdx + UNRELEASED_HEADING.length;
    const nextHeadingIdx = findNextHeadingIndex(content, afterHeadingIdx);
    const body = content.slice(afterHeadingIdx, nextHeadingIdx);
    return body
 ...
 */
export function hasReleasedHeading(cwd, version) {
    const content = readChangelog(cwd);
 ...
 */
export function previewChangelog(cwd, target, date) {
    const content = readChangelog(cwd);
 ...
 */
export function consolidateChangelog(ctx, target, date) {
