...
const DOKS_SYNC_FILES = ["CLAUDE.md", "README.md"];
...
const COMMIT_ALLOWLIST = [...RELEASE_FILES, ...DOKS_SYNC_FILES];
 ...
 */
function isVersionAlreadyBumped(cwd, targetVersion) {
    try {
 ...
 */
export function planRelease(ctx, deps) {
    ...
    if (!preflight.ok) {
        ...
        const recovering = isVersionAlreadyBumped(ctx.cwd, nextVersion) &&
 ...
 */
export function applyReleaseWrites(ctx, plan, _deps) {
 ...
 */
function commitRelease(ctx, plan) {
 ...
 */
function isReleaseWritesComplete(ctx, plan) {
    return (isVersionAlreadyBumped(ctx.cwd, plan.nextVersion) &&
        hasReleasedHeading(ctx.cwd, plan.nextVersion));
}
export function commitAndTag(ctx, plan, _deps) {
    if (plan.blocked.length > 0) {
        return [blockedResult(plan.blocked)];
    }
        ...
        return [
            {
                step: "commit",
                status: "failed",
                message: "Refusing to commit: release writes incomplete (version bump or CHANGELOG consolidation did not land)",
                filesTouched: [],
