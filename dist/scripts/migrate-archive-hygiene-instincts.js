...
export const DELETED_HYGIENE_PATTERN_NAMES = [
...
export function runArchiveMigration() {
    ...
    const selectStmt = db.prepare(`SELECT id FROM instincts WHERE status = 'active' AND pattern IN (${placeholders})`);
    ...
    const updateStmt = db.prepare("UPDATE instincts SET status = 'archived', updated_at = @updated_at WHERE id = @id");
...
async function main() {
    ...
    try {
        ...
        process.stderr.write(`migrate-archive-hygiene-instincts: archived ${archivedIds.length} instincts\n`);
        if (archivedIds.length > 0) {
...
if (invokedDirectly) {
    main().catch((err) => {
        process.stderr.write(`migrate-archive-hygiene-instincts failed: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    });
}
