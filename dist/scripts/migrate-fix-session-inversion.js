...
export function runMigration(options) {
    ...
    db.prepare(`UPDATE sessions
     ...
     WHERE ended_at IS NOT NULL AND ended_at < started_at`).run();
    return {
        invertedCount,
...
async function main() {
    ...
    try {
        ...
        if (result.dryRun) {
            console.log(`[dry-run] DB: ${dbPath ?? "~/.kadmon/kadmon.db (default)"}`);
            console.log(`[dry-run] ${result.invertedCount} inverted session rows found (ended_at < started_at).`);
            if (result.invertedCount > 0) {
             ...
             ORDER BY started_at DESC LIMIT 3`)
                ...
                for (const row of samples) {
                    console.log(`  id=${String(row.id)}  started_at=${String(row.started_at)}  ended_at=${String(row.ended_at)}`);
                }
                console.log(`[dry-run] Run with --apply to repair ${result.invertedCount} row(s).`);
            }
            else {
...
if (invokedDirectly) {
    main().catch((err) => {
        process.stderr.write(`migrate-fix-session-inversion failed: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    });
}
