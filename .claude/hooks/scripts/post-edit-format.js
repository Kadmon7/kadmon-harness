...
try {
  ...
  try {
    execSync(`npx prettier --write "${fp}"`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
  } catch { /* prettier not installed or failed — non-blocking */ }
} catch (err) { console.error(JSON.stringify({ error: `post-edit-format: ${err.message}` })); }
process.exit(0);
