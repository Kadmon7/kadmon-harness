...
try {
  ...
  try {
    execSync('npx tsc --noEmit --skipLibCheck', { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
  } catch (tscErr) {
    if (tscErr.stdout) console.log(`TypeScript errors:\n${tscErr.stdout}`);
    if (tscErr.stderr) console.log(tscErr.stderr);
  }
} catch (err) { console.error(JSON.stringify({ error: `post-edit-typecheck: ${err.message}` })); }
process.exit(0);
