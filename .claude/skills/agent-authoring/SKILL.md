...
| `fable-prompt` | Operator, directly via the Skill tool — no command or agent owner | Not a command-level skill in the strict sense (there is no command to attach it to) but the same "intentional orphan" logic applies. Fable 5 prompt-authoring is a manual, ad-hoc step the operator takes before kicking off an autonomous run — it doesn't fit any deterministic pipeline. Added `f912181` 2026-07-06; documented here so future orphan audits recognize it as by-design rather than accidental. |
