...
- All 22 script-backed hooks use `PATH="$PATH:/c/Program Files/nodejs"` prefix for Node.js resolution (the inline graphify nudges have no script file, so no prefix)
...
- session-end-all.js — consolidated Stop hook: persist session + daily log + evaluate patterns + track cost + persist hook events & agent invocations + write marker + cleanup + /evolve cadence nudge (Phase 6, AUD-26) (imports generate-session-summary, evaluate-patterns-shared, daily-log as shared modules)
