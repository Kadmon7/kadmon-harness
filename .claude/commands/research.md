...
- `<youtube-url>` — a single YouTube URL; skavenger extracts the transcript via yt-dlp and synthesizes from it (e.g., `/research https://www.youtube.com/watch?v=<id>`)
- `<pdf-url>` or `<arxiv-url>` — a PDF or arXiv paper URL; skavenger fetches and summarizes
- `<topic with url>` — mixed free-text with one or more URLs; skavenger treats URLs as primary sources and uses search to fill gaps

## Steps
1. Invoke skavenger agent with the raw argument
2. skavenger classifies input and chooses Route A (YouTube), B (PDF/arXiv), or C (general deep-research workflow)
3. skavenger executes the `deep-research` skill's Steps 2–6 (with Route A/B shortcuts) under the execution caps from ADR-009 D5
4. skavenger returns a cited report with TL;DR, themes, key takeaways, sources list, and Methodology footer
...
## Prerequisites
- For YouTube transcripts: `yt-dlp` must be on PATH. Install with `winget install yt-dlp` (Windows), `brew install yt-dlp` (macOS), or `pip install yt-dlp` (cross-platform). If missing, skavenger degrades gracefully and continues with WebFetch metadata.
...
Result:
## Research: [Video Title] [skavenger]
