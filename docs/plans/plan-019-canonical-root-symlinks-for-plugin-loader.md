...
|---|---|---|---|
| Windows `core.symlinks = false` causes `ln -s` to create text files, git commits non-symlinks | Medium (Abraham + Ych-Kadmon are Windows) | High — plugin loader would see text files, not directories | A.2 verify step catches this locally; README "Installing on Windows" section (part of plan-010 Phase 7) documents the one-time setup; plan-019 also assumes Developer Mode is already ON on the dev machine per ADR-019 pre-conditions |
