---
number: 18
title: Port 3 ECC Features + Phase 2 ruled out (decay, rubric, cross-project promotion)
date: 2026-04-19
status: completed
needs_tdd: true
route: B
...
# Plan 018: Port ECC Features [konstruct]

## Overview

Port features from ECC's `continuous-learning-v2` Python skill into the Kadmon Harness TypeScript pipeline. **3 of 4 features shipped**; Phase 2 (`user_correction` pattern detector) **ruled out** during pre-implementation research — see the Phase 2 section for the definitive rationale.

Shipped (commits `7ed7051`, `2bdc982`, `d51dc0e` on 2026-04-19):
1. Confidence decay at `-0.02/week` for silent instincts — Phase 1
3. 1-paragraph rubric insertion in `alchemik.md` — Phase 3
4. Cross-project auto-promotion of instincts observed in 2+ projects with avg confidence >= 0.8 — Phase 4

Not shipped:
2. `user_correction` pattern detector — **ruled out** (Claude Code hook API constraint, see Phase 2 below).
...
- ~~`pattern-definitions.json` currently has 12 definitions — adding a 13th `user_correction` definition does not break the loader; `PatternDefinition` union in `types.ts` covers 5 types~~ **Superseded**: Phase 2 ruled out (see Phase 2 section). `pattern-definitions.json` stays at 12 entries; `PatternDefinition` union stays at 5 variants.
...
## Phase 2: User-Correction Pattern Detector — RULED OUT (not deferred)

**Status: ruled out on 2026-04-19.** Not deferred, not pending, not "maybe later" — the blocker is external and definitive.

### Root cause

Pre-implementation research (feniks, 2026-04-19) confirmed that **Claude Code's `PreToolUse` hook API does not emit user-message events**. `.claude/hooks/scripts/observe-pre.js` receives only tool inputs — the raw text of the user's turn is never exposed to the hook. Sample event shape from real observations.jsonl:

```json
{"timestamp":"...","sessionId":"...","eventType":"tool_pre","toolName":"Bash","filePath":null,"metadata":{"command":"..."}}
```

There is no `eventType: "user_message"` or equivalent. Without a signal source, the marker-based detector design of Phase 2 — matching strings like `"no así"` / `"stop"` / `"esto está mal"` against user turns — cannot be implemented.

### Alternatives considered and rejected

- **Fallback approach-change detector (tool-sequence shift proxy)**. Detect "corrections" heuristically by comparing the 5 tool calls before vs after a potential pivot point. Rejected because it produces high false-positive rates on normal phase transitions (research → planning, debug → feature work). Polluting the instinct DB with low-signal entries directly undermines the harness's core value proposition — that captured patterns should be trustworthy.

- **Extend `observe-pre.js` to capture user turns**. Would require Claude Code's hook API to surface user turn text, which it does not in the current schema. Not an engineering effort we can complete inside the harness — it's a capability gap upstream.

### Decision

No code was written for Phase 2. No entries were added to `.claude/hooks/pattern-definitions.json`. No variant was added to the `PatternDefinition` union in `scripts/lib/types.ts`. The user_correction pattern type does not exist in the harness and will not be added under the current Claude Code hook schema.

**What would change this decision**: if Claude Code introduces a new hook event type (e.g. `UserPromptSubmit` payload exposing turn text), revisit. Until then: not pending, ruled out.

### Original design (preserved for historical reference)

The original plan proposed: (1) add a 6th variant to the `PatternDefinition` union with a `correctionMarkers: string[]` field, (2) implement `detectUserCorrectionPattern` in `pattern-engine.ts` walking observations for marker matches within a 5-call window, (3) add a 13th definition to `pattern-definitions.json`. This design assumed observations.jsonl contained user-message events. It did not.
...
### Unit
- `tests/lib/instinct-manager.test.ts` — new `describe("decayInstincts", ...)` block (7 tests, incl. isNaN fallback + projectHash scoping + legacy pre-v0.5 fallback), new `describe("promoteToGlobal", ...)` block (4 tests).
- ~~`tests/lib/pattern-engine.test.ts` — new `describe("detectUserCorrectionPattern", ...)` block~~ (Phase 2 ruled out — tests not written).
- `tests/lib/cross-project-promotion.test.ts` — new file, 12 tests for the candidate query (plan's 5 + feniks-identified gaps: promoted-exclusion, contradictions, 0.800 boundary, dup-in-project, parameter overrides).
...
### Regression
- `npx vitest run` full suite — baseline 610 passing grew to 637 (+27 tests across the 3 shipped phases).
- `npx tsc --noEmit` must stay green at every commit. Verified across all 3 commits.
...
- ~~**Risk**: `detectUserCorrectionPattern` produces false positives from codebase-mentioned strings.~~ **Resolved**: Phase 2 ruled out, no detector was implemented.
...
- ~~**Risk**: Pattern-engine regression from adding a 6th variant to the union.~~ **Resolved**: Phase 2 ruled out, no variant was added — `PatternDefinition` stays at 5 variants.
...
- [x] `instincts` table has `last_observed_at TEXT` column on both fresh installs (via schema.sql) and existing installs (via migrate-v0.5.ts). **Shipped in `7ed7051`.**
- [x] `decayInstincts(projectHash)` reduces active-instinct confidence by 0.02 per full week since `last_observed_at`, clamped to `[0, originalConfidence]`. **Shipped in `7ed7051`.**
- [x] `/forge` reinforcement writes `last_observed_at = nowISO()` to both created and reinforced instincts. **Shipped in `7ed7051`.**
- [ ] ~~`.claude/hooks/pattern-definitions.json` has 13 entries~~ — **N/A, Phase 2 ruled out**; stays at 12 entries.
- [ ] ~~`evaluatePatterns` handles `user_correction`~~ — **N/A, Phase 2 ruled out**.
- [x] `alchemik.md` contains the "Rubric: which artifact to propose?" table between Evolution Categories and Output Format. **Shipped in `2bdc982`.**
- [x] `/forge` preview gate includes a "Would promote (scope change)" section when 2+ projects share an instinct at avg conf >= 0.8. **Shipped in `d51dc0e`.**
- [x] `applyForgePreview` changes `scope: 'project' -> 'global'` for approved cross-project promotions. **Shipped in `d51dc0e`.**
- [x] `npx vitest run` passes: 637 tests (baseline 610, delta +27).
- [x] `npx tsc --noEmit` passed at every commit.
- [x] All 3 shipped commits follow conventional-commit format + `Reviewed: <tier>` footer.

## Commit Strategy

Actual shipping (2026-04-19):

| # | Phase | Commit | Tier | Status |
|---|-------|--------|------|--------|
| 1 | Phase 1 | `7ed7051` `feat(instincts): add last_observed_at column + confidence decay (ECC port 1/4)` | full | shipped |
| — | Phase 2 | `feat(pattern-engine): add user_correction detector (ECC port 2/4)` | — | **ruled out** (see Phase 2 section) |
| 2 | Phase 3 | `2bdc982` `docs(alchemik): add artifact-choice rubric to evolution categories (ECC port 3/4)` | skip | shipped |
| 3 | Phase 4 | `d51dc0e` `feat(forge): cross-project instinct promotion via scope change (ECC port 4/4)` | full | shipped |

Phase 1 shipped schema + decay. Phase 3 shipped the rubric. Phase 4 shipped cross-project promotion. Phase 2 was ruled out during pre-implementation research — no commits, no code, no partial state in the repo. The three shipped phases are independently mergeable; none depends on Phase 2.
