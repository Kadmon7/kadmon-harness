...
## Post-implementation note (2026-04-17, skavenger.md trim pass)

Skavenger.md trimmed from 436 → 351 lines (-85, -19.5%) after observation that the prompt was 2x the project convention for agent files (~150-200 lines). Route B (PDF/arXiv as a dedicated branch in the Step 1 classifier) was **consolidated into Route C as inline preprocessing** — same capability (if input URL matches `\.pdf` or `arxiv.org`, WebFetch first and skip sub-question decomposition), but no longer its own branch in the classifier. Triggered by user-confirmed low-usage data ("no la uso todavía"). No ADR-016 opened because the semantic capability is preserved exactly — only the prompt structure changed. If Route B usage pattern ever emerges that warrants its own dedicated classifier branch again, reintroduce as ADR-016. Additional cuts in the same pass: 4 Examples → 2 (kept Route C + Route A), dormant Perplexity Fase 2 comment removed, inline self-eval TODO compressed, Memory section boilerplate compacted, F9/F10/Part 3/--continue prose tightened. Zero capability lost.
