...
- [ ] AUD-39 `quality-gate.js` per-edit latency: reviving the ESLint 9 probe (AUD-35) made every `.ts` edit under `tests/**|scripts/**` pay ~2.2s type-aware lint (`projectService`), ~0.9s outside — within the documented toolchain-hook exception, but a real felt cost stacked on post-edit-typecheck's tsc spawn. Decide: keep type-aware, switch to a lightweight non-type-aware invocation, make the hook async/non-blocking, or drop eslint-on-every-edit (post-edit-typecheck already catches type errors). ts-reviewer WARN, Wave 3. Secondary: dormant ignore-pattern noise footgun — a future root `.js` or a consumer with an ignore-heavy flat config would print "File ignored…" on every edit (inert today, only `eslint.config.js` matches).
