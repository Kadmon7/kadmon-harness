...
- [x] AUD-35 (SHIPPED `d56e2b8`) `quality-gate.js` `--no-eslintrc` is invalid under ESLint 9 (flat config) — errors at CLI-arg parse, and `runEslint` only forwards stdout (not stderr), so ESLint findings never surface. Pre-existing; surfaced by Wave 3 spektr/ts-reviewer. Quality gap not security (the probe only ran `no-unused-vars`, zero security rules). Fix: drop `--no-eslintrc`, use a flat-config-compatible invocation, forward stderr.
- [x] AUD-36 (SHIPPED `d56e2b8`) `path.resolve(fp)` parity — `runEslint` (quality-gate.js) and `runPrettier` (post-edit-format.js) pass raw `fp`; `runRuff` already hardens with `path.resolve()`. Pre-existing low-risk parity gap (spektr LOW, Wave 3).
- [x] AUD-37 (SHIPPED `3e3ec3b`) `scripts/lib/state-store.ts` was 1201 lines (>800 hard limit). Split into a 21-line barrel facade + 8 modules under `scripts/lib/state-store/`: core (sql.js wrapper + DB singleton + schema apply/migrations + shared parseJson), sessions, instincts, cost-events, sync-queue, hook-events, agent-invocations, research-reports. Byte-for-byte behavior-preserving move + 1 intentional edit (openDb schema.sql `../` path, core is one dir deeper); all 36 exports preserved, ~65 import sites unchanged, parseJson/WrappedDb stay package-internal, largest module 286 lines. `clearSessionEndState` -> sessions.ts; FTS5 cache -> research-reports.ts (single owner). konstruct-planned; full-tier /chekpoint (spektr + orakle + typescript-reviewer + kody all GO, 0 BLOCK); suite 1385/1385. Also `55c7b6d`: hardened `orchestrate.e2e.test.ts` (structural invariant, not live tree state — 2nd occurrence of that class).
- [x] AUD-38 (SHIPPED `d56e2b8`) Wave 3 review NOTE batch (all low): scrub-secrets JSDoc caveat for a future 3rd caller past the 16KB cap; `scanDiskMaxReportNumber` malformed-filename regression test; `quality-gate` npx-fallback test can stall offline (20s timeout); `binProjectRoot` nested-`node_modules` cache-anchor edge; `migrate-v0.6.ts` self-referential statements always hit the already-migrated branch (intentional, documented).
- [x] AUD-39 (SHIPPED `975d5d4`) `quality-gate.js` per-edit latency: reviving the ESLint 9 probe (AUD-35) made every `.ts` edit under `tests/**|scripts/**` pay ~2.2s type-aware lint (`projectService`), ~0.9s outside. DECISION: lightweight non-type-aware invocation — removed `projectService`/`tsconfigRootDir` from `eslint.config.js` (no active rule needs type info: the two configured rules plus all of `tseslint.configs.recommended` are syntactic; `recommended`, not `recommendedTypeChecked`). Measured: single-file 2.82s→1.32s (-53%), full lint 4.93s→3.22s (-35%), identical findings (zero-loss). Blast radius harness-only (config not shipped to consumers). Secondary FIXED: `--no-warn-ignored` added to the hook (both resolveBin + npx branches) kills the dormant ignore-pattern noise footgun (root `.js` / ignore-heavy consumer config). TDD via feniks; ts-reviewer GO (0 BLOCK/0 WARN, 3 NOTEs applied). Suite 1355/1355.

## P2 — features / trims

- [x] AUD-24 /release command — SHIPPED (ADR-037 accepted + plan-037 completed): `scripts/lib/release/` (types + 7 modules) + `release.md` (Direct no-agent) + count 11→12 + CHANGELOG/BACKLOG/tag + `/doks` composition. Full-tier /chekpoint: spektr GO (0C/0H/1M/3L) + ts-reviewer GO (no BLOCK); 6 findings repaired via feniks TDD (R1 git-add pathspec, R2 hollow-release guard, R3 unconditional previews, R4 typed IO error, R5 partial-commit guard, R6 interface). The 4AM autonomous framing was RETIRED (ADR-037 Amendment 2026-07-13) — /release is human-invoked, no-push default, propose-only status flips. Follow-up: AUD-40.
