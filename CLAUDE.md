...
## Status
v1.2.3-dev — Bug 3 regression follow-up 2026-04-22: the v1.2.2 Node `os.tmpdir()` approach returned Windows `C:\...` backslash paths that got mangled by xargs + MSYS path conversion on Git Bash. Fixed in kompact.md via env-var chain `"${TMPDIR:-${TEMP:-/tmp}}"` that coincides with Node's write location on every shell that can run bash pipelines (Mac `$TMPDIR`, Linux `/tmp`, Git Bash `$TEMP=/tmp`). Also fixed a latent kompact.md grep regex bug: field name was `file_path` (snake_case) but observations.jsonl uses `filePath` (camelCase) — regex had never matched. v1.2.2 shipped: Bug 4 (commit-format-guard.js strips quoted strings before matching `git commit`, eliminates `echo "...git commit..."` false-positives). v1.2.1: Bug 1 (hook_events dedup via UNIQUE INDEX + ON CONFLICT DO NOTHING) + Bug 2 (orphan recovery staleness guard + endSession project_hash assert). All 4 original bugs detected from Kadmon-Sports dogfood 2026-04-22.
