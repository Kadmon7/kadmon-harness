...
**Enforced by hooks** (always visible via stderr):
- 🪝 [hook-name]: [what it did] — hooks write to stderr automatically

**Best-effort** (depends on Claude following these instructions — not enforceable by hooks):
- 🤖 [agent-name]: [reason]
...
**Automatic** (generated by commands/scripts):
- ✅ Verify passed: [check]
- ❌ Verify failed: [check]
- 💰 Cost: [amount]
- 📋 Plan: [file created]

Note: Hooks can remind but cannot force text output format. Best-effort items rely on Claude compliance with CLAUDE.md.
