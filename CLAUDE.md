...
## Status
v1.2.1 — latest shipped: Bug 1 (hook_events dedup via UNIQUE INDEX + ON CONFLICT DO NOTHING) + Bug 2 (orphan recovery staleness guard + endSession project_hash assert), detected from Kadmon-Sports dogfood 2026-04-22. Previous: v1.2.0 plan-020 runtime language detection (ADR-020, 2026-04-21).
Metrics: 894 tests / 73 files / 21 hooks / 16 agents / 46 skills / 11 commands / 19 rules / 7 DB tables.
