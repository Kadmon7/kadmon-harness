...
function addObservation(toolName: string, filePath: string): void {
  ...
  fs.appendFileSync(OBS_FILE, JSON.stringify({ toolName, filePath, eventType: 'tool_pre' }) + '\n');
}
...
describe('no-context-guard', () => {
  ...
  it('blocks Write when no Read was performed', () => {
    ...
    const r = runHook({ session_id: SESSION_ID, tool_name: 'Write', tool_input: { file_path: 'src/foo.ts' } });
  ...
  it('allows Write for test files', () => {
    fs.writeFileSync(OBS_FILE, '');
    const r = runHook({ session_id: SESSION_ID, tool_name: 'Write', tool_input: { file_path: 'src/foo.test.ts' } });
    expect(r.exitCode).toBe(0);
  });

  it('allows Write for markdown files', () => {
    fs.writeFileSync(OBS_FILE, '');
    const r = runHook({ session_id: SESSION_ID, tool_name: 'Write', tool_input: { file_path: 'docs/README.md' } });
    expect(r.exitCode).toBe(0);
  });
  ...
  it('allows Write when KADMON_NO_CONTEXT_GUARD=off', () => {
    fs.writeFileSync(OBS_FILE, '');
    const r = runHook(
      { session_id: SESSION_ID, tool_name: 'Write', tool_input: { file_path: 'src/foo.ts' } },
