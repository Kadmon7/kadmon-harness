...
import { openDb, closeDb, upsertInstinct } from '../../scripts/lib/state-store.js';
import { startSession, endSession, getLastSession, loadSessionContext } from '../../scripts/lib/session-manager.js';
...
describe('session-manager', () => {
  ...
  it('loadSessionContext returns markdown with session and instinct info', () => {
    ...
    upsertInstinct({
      id: 'inst-1', projectHash: PROJECT.projectHash,
      pattern: 'always run tsc', action: 'tsc --noEmit', confidence: 0.7,
    });
